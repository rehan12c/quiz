<template>
    <div class="sosmed"></div>
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark text-ligth">   
        <!--card awal-->
    <div class="row text-dark mt-4">
  <div class="col-sm-4 g-4">
    <div class="card">
      <div class="card-body">
        <h5 class="card-title text-center">Instragram</h5>
        <center> <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/5/58/Instagram-Icon.png/600px-Instagram-Icon.png?20190314235631" alt="img" style="height: 200px; width: 200px;"></center>
        <center><p class="card-text">Kunjungi Instragram</p></center>
        <a href="https://www.instagram.com/rehannugraha06/" class="btn btn-dark d-block">Go To Instragram</a>
      </div>
    </div>
  </div>
  <div class="col-sm-4 g-4">
    <div class="card">
      <div class="card-body">
        <h5 class="card-title text-center">Github</h5>
        <img src="https://cdn.icon-icons.com/icons2/2368/PNG/512/github_logo_icon_143772.png" alt="img" style="width: 200px; height: 200px;">
        <center><p class="card-text">Kunjungi Github</p></center>
        <a href="https://github.com/rehan12c" class="btn btn-dark d-block">Go To Github</a>
      </div>
    </div>
  </div>
  <div class="col-sm-4 g-4">
    <div class="card">
      <div class="card-body">
        <h5 class="card-title text-center">Gmail</h5>
        <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/7/7e/Gmail_icon_%282020%29.svg/512px-Gmail_icon_%282020%29.svg.png?20221017173631" alt="img" style="height: 200px; width: 200px;">
        <center><p class="card-text">Kunjungi Gmail</p></center>
        <a href="mailto:rehannugraha64@gmail.com" class="btn btn-dark d-block">Go To Gmail</a>
      </div>
    </div>
  </div>
</div>
    <!--card akhir-->
    </nav>  
    
</template>